...
class Projects_model extends Super_Model
{
    ...
    function allProjects($orderby = 'projects_title', $sort = 'ASC', $clients_id = '', $status = 'all')
    {
        ...
        $query = $this->db->query("SELECT *,services.services_name as project_service
                                          FROM projects
                                          LEFT JOIN services on services.services_id = projects.projects_service 
                                          WHERE 1 = 1
                                          $conditional_sql
    ...
    function searchProjects($offset = 0, $type = 'search', $clients_id = '', $status = '')
    {
        ...
        if ($this->input->get('projects_service')) {
            $service_id = $this->input->get('projects_service', true);
            $conditional_sql .= " AND projects_service={$service_id}";;
        }
        ...
        $sort_columns = array(
            ...
            'sortby_projecttitle' => 'projects.projects_title',
            'sortby_projectservice' => 'projects.projects_service',
            'sortby_duedate' => 'projects.projects_end',
    ...
    function projectDetails($id = '')
    {
        ...
        //_____SQL QUERY_______
        $query = $this->db->query("SELECT projects.*, clients.*, team_profile.*,services.*,
