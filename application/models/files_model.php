...
class Files_model extends Super_Model
{
    ...
    function addFile($id = '')
    {
        ...
        $files_islink = $this->input->post('files_islink', true) ? 1 : 0;;

        $files_size_human = $this->data['vars']['files_size_human'];

        $fields = array('files_client_id', 'files_description', 'files_events_id', 'files_extension', 'files_foldername', 'files_name', 'files_project_id', 'files_uploaded_by', 'files_uploaded_by_id', 'files_date_uploaded', 'files_size', 'files_size_human', 'files_time_uploaded', 'files_link', 'files_islink', 'files_videotype');
        $data = array();
        foreach ($fields as $field) {
            if (isset($$field) && !is_null($$field)) {
                $data[$field] = $$field;
            } elseif ($this->input->post($field, true) != false) {
                $data[$field] = $this->input->post($field, true);
            } else {
                $data[$field] = '';
            }
        }
        $data['files_date_uploaded'] = date('Y-m-d');
        $data['files_time_uploaded'] = date('Y-m-d');
        ...
        //_____SQL QUERY_______
        /* $query = $this->db->query("INSERT INTO files (
                                           files_client_id,
                                           files_description,
                                           files_events_id,
                                           files_extension,
    ...
    {
        $file = $this->db->where('files_id', $id)->get('files');
        if ($file->num_rows() == 0) {
            return false;
        }
