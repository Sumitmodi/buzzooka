...
class Files extends MY_Controller
{
    ...
    function __filesView()
    {
        ...
        ** -----------------------------------------*/
        ...
        $this->data['blocks']['files'] = $files = $this->files_model->searchFiles($offset, 'search', $this->project_id);
        ...
        $this->data['reg_blocks'][] = 'cats';
        $this->data['blocks']['cats'] = $cats;
    }
    ...
    function __prepFilesView($thedata = '')
    {
        ...
        *------------------------------------------------------------------------------------*/
        for ($i = 0; $i < count($thedata); $i++) {
            ...
            if ($thedata[$i]['files_islink'] == 1) {
                $thedata[$i]['files_extension'] = $thedata[$i]['files_videotype'];
            }
            $thedata[$i]['image_url'] = site_url(sprintf('/files/filetype_icons/%s.png', $thedata[$i]['files_extension']));

            if (empty($thedata[$i]['files_name'])) {
                $thedata[$i]['files_name'] = $thedata[$i]['files_description'];
            }
            ...
            if ($thedata[$i]['files_islink']) {
                ...
                $thedata[$i]['file_url'] = site_url('files/projects/' . $thedata[$i]['files_project_id'] . '/' . $thedata[$i]['files_foldername'] . '/' . $thedata[$i]['files_name']);
                if (in_array($thedata[$i]['files_extension'], array('jpg', 'rpg', 'png', 'tiff', 'jpeg'))) {
                    $thedata[$i]['file_type_id'] = str_replace(' ', '-', 'images');
                    $thedata[$i]['image_url'] = $thedata[$i]['file_url'];
                    $thedata[$i]['has_preview'] = 1;
                } else {
                    $thedata[$i]['has_preview'] = 0;
                    $thedata[$i]['file_type_id'] = str_replace(' ', '-', 'videos');
                }
            }

            //file preview url
            if (in_array($thedata[$i]['files_extension'], array('doc', 'docx', 'ppt', 'pptx', 'xls', 'xlsx'))) {
    ...
    function __filesAdd()
    {
        ...
        $upload_type = $this->input->post('files_islink', true) ? 1 : 0;

        //validate form & display any errors
        if (!$this->__flmFormValidation($upload_type ? 'upload-video' : 'add_file')) {
        ...
        if ($next) {
            ...
            $hidden_fields = array(
                ...
                'files_uploaded_by_id' => 'numeric'); //loop through and validate each hidden field
            if (!$upload_type) {
                $hidden_fields['files_size'] = 'numeric';
                $hidden_fields['files_foldername'] = 'string';
                $hidden_fields['files_extension'] = 'string';
            }
        ...
        if ($next && $upload_type) {
            $url = parse_url($_POST['files_link']);
            if (in_array($url['host'], array('www.youtube.com', 'youtube.com'))) {
                $query = $url['query'];
                parse_str($query);
                if (isset($v)) {
                    $_POST['files_link'] = 'https://youtube.com/embed/' . $v;
                    $_POST['files_videotype'] = 'youtube';
                }
            } elseif (in_array($url['host'], array('www.vimeo.com', 'vimeo.com'))) {
                $path = explode('/', ltrim($url['path'], '/'));
                $_POST['files_link'] = 'https://player.vimeo.com/embed/' . $path[0];
                $_POST['files_videotype'] = 'vimeo';
            } else {
                $_POST['files_videotype'] = 'link';
            }
        }
        ...
        if ($next) {
            ...
            //human file size
            $this->data['vars']['files_size_human'] = $upload_type ? 0 : convert_file_size($this->input->post('files_size'));
            ...
            if ($result = $this->files_model->addFile()) {
                ...
                //email notification
                $this->__emailer('mailqueue_new_file');
    ...
    function __flmFormValidation($form = '')
    {
        ...
        if ($form == 'upload-video') {

            //check required fields
            $fields = array('files_description' => $this->data['lang']['lang_description'], 'files_link' => $this->data['lang']['lang_video_url']);
