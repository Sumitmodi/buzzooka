...
class Report extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        //default page title
        $this->data['vars']['main_title'] = $this->data['lang']['lang_project_report'];
    ...
    function __fileView()
    {
        ...
        if ($next) {
            ...
            //get file details
            if ($this->data['fields']['file'] = $this->model->getFile($id)) {
        ...
        if ($next) {
            ...
            if (in_array($this->data['fields']['file']['reports_extension'], $preview_extensions)) {
                $this->data['visible']['wi_file_preview_image'] = 1;
            }else if (in_array($this->data['fields']['file']['reports_extension'], $video_extensions)){
                $this->data['visible']['wi_file_preview_video'] = 1;
            }
            else {
    ...
    function __prepFileView($thedata = '')
    {
        ...
        *------------------------------------------------------------------------------------*/
        ...
        //-------------------(2) ADD FILE URL---------------------------------------\\
        $thedata['file_url'] = site_url('files/projects/' . $thedata['reports_project_id'] . '/' . $thedata['reports_foldername'] . '/' . $thedata['reports_name']);

        //-------------------(3) ADD FILE PATH---------------------------------------\\
        $thedata['file_path'] = FILES_PROJECT_FOLDER . $thedata['reports_project_id'] . '/' . $thedata['reports_foldername'] . '/' . $thedata['reports_name'];
    ...
    function __fileDownload()
    {
        ...
        if ($next) {
            if ($result = $this->model->getFile($file_id)) {
        ...
        if ($next) {
            ...
            if (is_file($file_path)) {
                ...
                $this->model->downloadCounter($file_id);
